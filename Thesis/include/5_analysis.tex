...
The following results and studies were based upon the AFEW-VA dataset. Results were compared with the metrics Root-Mean-Squared-Error (RMSE) and Pearson Correlation (CORR). The optimization goal was to reduce the RMSE error while improving the CORR.

\section{Benchmark results}
In the original paper from 2017 where the AFEW-VA database \citep{Kossaifi:2017:AFEW-VADatabase} was introduced, \citet{Kossaifi:2017:AFEW-VADatabase} already provided a baseline by comparing different methods such as SVR (Support Vector Machine - Regression) or DCNN (Deep Convolutional Neural Network). In the following table, the best performing deep neural network approach is compared to the overall best performing algorithm, called Multiple Kernel Learning (MKL):
...
\begin{tabular}[c]{@{}l@{}}FT-DCNN\\ (RGB images)\end{tabular} & Valence                     & 0.37           & 0.26          \\
\begin{tabular}[c]{@{}l@{}}FT-DCNN\\ (RGB images)\end{tabular} & \multicolumn{1}{l}{Arousal} & 0.39           & 0.31          \\
\begin{tabular}[c]{@{}l@{}}MKL\\ (Shape + DCT)\end{tabular}    & \multicolumn{1}{l}{Valence} & \textbf{0.264} & \textbf{0.40} \\
\begin{tabular}[c]{@{}l@{}}MKL\\ (Shape + DCT)\end{tabular}    & Arousal                     & \textbf{0.223} & \textbf{0.45} \\ \bottomrule
\end{tabular}
\caption{Multiple Kernel Learning (MKL) algorithm substantially outperforms the Fine-Tuned Deep Convolutional Neural Network (FT-DCNN) in the AFEW-VA paper \citep{Kossaifi:2017:AFEW-VADatabase}}
\label{tab:BenchmarkAFEWVA}
\end{center}
\end{table}

The best performing method in the aforementioned study was the MKL algorithm.The Fine-Tuned Deep Convolutional Neural Network approach is very similar to the one used in this thesis, which is trained on randomly sampled frames from video sequences. Fine-tuning is being done with AlexNet, a pretrained model on the ImageNet dataset.
\newline\newline
...
\toprule
\multicolumn{1}{c}{}                                          &         & RMSE $\downarrow$          & CORR $\uparrow$          \\ \midrule
\begin{tabular}[c]{@{}l@{}}AFEW-VA\\ DB only\end{tabular}     & Valence & \textbf{0.26} & 0.39          \\
                                                              & Arousal & \textbf{0.25} & 0.29          \\ \midrule
\begin{tabular}[c]{@{}l@{}}Cross-DB\\ validation\end{tabular} & Valence & 0.28          & \textbf{0.58} \\
                                                              & Arousal & 0.26          & \textbf{0.46} \\ \bottomrule
\end{tabular}
\caption{'Simultaneous VA prediction'
\citep{Handrich:2020:SimultaneousPredVA} also outperforms the baseline (AFEW-VA paper \citep{Kossaifi:2017:AFEW-VADatabase}) with their approach. Additionally, their Cross-DB validation could achieved a strong improvement in terms of CORR over their validation on the AFEW-VA dataset only.}
...
%   \caption{Despite the Cross-database approach of the paper 'Simultaneous Prediction of Valence/Arousal and Emotions on AffectNet, Aff-Wild and AFEW-VA' \citep{Handrich:2020:SimultaneousPredVA} outperforming all the other approaches, it cannot be objectively compared to the AFEW-VA paper \citep{Kossaifi:2017:AFEW-VADatabase}, as it was trained on a different dataset.}
...
\begin{tabular}[c]{@{}l@{}}best result\\ (without 5-fold-CV)\end{tabular} & 0.24 & 0.22 & 0.38 & 0.46 \\
